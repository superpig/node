...
class Worker extends EventEmitter {
  ...
  [kOnExit](code, customErr, customErrReason) {
    ...
    console.error('workerlog=== kOnExit start')
    drainMessagePort(this[kPublicPort]);
    console.error('workerlog=== kOnExit drain port')
    drainMessagePort(this[kPort]);
    console.error('workerlog=== kOnExit dispose')
    this[kDispose]();
    if (customErr) {
      console.error(`[${threadId}] failing with custom error ${customErr} \
        and with reason {customErrReason}`);
      this.emit('error', new errorCodes[customErr](customErrReason));
    }
    console.error('workerlog=== kOnExit before emit exit')
    this.emit('exit', code);
    console.error('workerlog=== kOnExit after emit exit')
  ...
  terminate(callback) {
    ...
    console.error('workerlog=== terminate worker start')
    this.ref();
    console.error('workerlog=== terminate worker after ref')
    ...
    console.error('workerlog=== terminate worker stopThread')  
    this[kHandle].stopThread();
    console.error('workerlog=== terminate worker stopThread end')
    ...
    return new Promise((resolve) => {
      console.error('workerlog=== terminate worker before exit')
      this.once('exit', resolve);
      console.error('workerlog=== terminate worker after exit')
    });
  }
  ...
  ref() {
    ...
    console.error('workerlog=== ref() worker ref start')
    this[kHandle].ref();
    console.error('workerlog=== ref() worker port ref start')
